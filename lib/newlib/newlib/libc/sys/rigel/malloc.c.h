...
#ifdef RIGEL
//Undefine x86-related macros, so that we don't get x86 assembly in our Rigel .S files.
//These macros should only be defined when compiling for x86 targets, but they are also enabled by llvm-gcc under
//LLVM 2.1 and 2.5 for us because llvm-gcc is not Rigel-aware, and generates LLVM IR as if we were x86 (including inline ASM parsing).
#undef __i386__
#undef __x86_64__
#include "rigel.h" //For RigelAbort(), etc.
#include "stddef.h" //ptrdiff_t is actually in stddef.h, not sys/types.h.  Not sure why dlmalloc doesn't #include this for all platforms.
#define malloc_getpagesize (1U << 20)
#define MALLOC_ALIGNMENT ((size_t)8U)
#ifndef ABORT
  #define ABORT  RigelAbort()
#endif
#define ABORT_ON_ASSERT_FAILURE 1
#define USE_LOCKS 2 //Needs to be above 1 so dlmalloc will use our lock implementation instead of pthread mutexes.
#undef SPIN_LOCKS_AVAILABLE
#define SPIN_LOCKS_AVAILABLE 1
#define HAVE_MMAP 1
#define HAVE_MORECORE 0
#define MORECORE_CONTIGUOUS 0
#define MMAP_CLEARS 0 //Let's not assume mmap() zeroes out the memory, to permit security/speed tradeoffs in mmap().
#define HAVE_MREMAP 0 //We may have this in the future, but don't want to bother for now.
#ifndef NO_MALLINFO //nedmalloc likes to predefine this.
  #define NO_MALLINFO 1 //Again, maybe later.
#endif
#define USE_BUILTIN_FFS 0 //We should write our own bit-scanning ASM.  For now, we'll go without it.
...
#if USE_LOCKS
#ifndef WIN32
#ifndef RIGEL //Rigel should have a pthread.h, but we don't use pthreads for locks.
#include <pthread.h>
#endif
...
#ifdef RIGEL
#include <sys/lock.h> //For libc lock macros
#include <rigel.h>    //For RigelGetThreadNum()
#define MLOCK_T               _LOCK_RECURSIVE_T
#define CURRENT_THREAD        RigelGetThreadNum()
//Need the (X, 0) because __lock_init_recursive returns void, we use INITIAL_LOCK() in an if statement and need it to return 0 on success.
#define INITIAL_LOCK(sl) (__lock_init_recursive(*(sl)), 0)
#define DESTROY_LOCK(sl) (0)
#define ACQUIRE_LOCK(sl) (__lock_acquire_recursive(*(sl)), 0) //Ditto on the (X, 0) here
#define RELEASE_LOCK(sl) __lock_release_recursive(*(sl))
#define TRY_LOCK(sl) __lock_try_acquire_recursive(*(sl))
