...
#define DEFAULTMAXTHREADSINPOOL 1
 ...
 #include <alloca.h>
 #include <pthread.h>
 //#define __RIGEL_MALLOC_DEBUG
 #ifdef __RIGEL_MALLOC_DEBUG
 #define DEBUG
 #define NEDMALLOC_DEBUG 1
 #define ENABLE_LOGGING 7 //FIXME HACK HACK HACK This is only for debug, it will slow us down a lot.
 #define ABORT do {nedflushlogs(NULL, NULL); exit(1); } while (0); //Flushes the syspool log data to nedmalloc.csv.
                                                                     //NOTE: We call exit() instead of RigelAbort() so that nedmalloc.csv (which is buffered),
                                                                     //is flushed to file before exiting the program.  The abort() function, and our implementation
                                                                     //of it, is not required to flush open file descriptors.
 #else
 #define NDEBUG
 #endif
 ...
 #define DEFAULT_GRANULARITY (32*1024*1024)
 #define DEFAULTMAXTHREADSINPOOL 8
 #define MAXTHREADSINPOOL 8
 #define THREADCACHEMAXCACHES 4096
 #define MAX_PATH 1024 //FIXME This is actually a nedmalloc bug, MAX_PATH only exists on windows but it's used unconditionally when ENABLE_LOGGING is defined
#endif
...
size_t (*sysblksize)(void *)=
	...
	_msize;
#elif defined(__linux__) || defined(RIGEL)
...
#else
#ifdef RIGEL
//NOTE I'd rather just have the cycle number than a delta from the last timestamp
typedef unsigned long timeCount; //Don't want to deal with i64s yet.
static timeCount GetTimestamp()
{
	return RigelGetCycle();
}
#else
#include <sys/time.h>
...
#endif
#ifdef RIGEL //NOTE timestamp is a uint32_t for us, we don't handle long longs well yet.
							fprintf(oh, "%u, 0x%p, %s, %d, %zu, 0x%p, %zu, 0x%x, 0x%p,\"%s\"\n",
#else
							fprintf(oh, "%llu, 0x%p, %s, %d, %zu, 0x%p, %zu, 0x%x, 0x%p,\"%s\"\n",
#endif
