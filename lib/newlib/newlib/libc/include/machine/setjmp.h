...
#if defined(__CYGWIN__) || defined(__rtems__) || defined (RIGEL) //NOTE hacked this so we get sigjmp_buf defs etc. for libunwind
#include <signal.h>
...
#if defined(__CYGWIN__) || defined(RIGEL) //NOTE hacked this so we get the function definitions instead of macros, so libunwind can override
