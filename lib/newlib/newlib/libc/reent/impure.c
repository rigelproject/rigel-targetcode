...
/* Redeclare these symbols locally as weak so that the file containing
   ...
   of memory.  */
#ifdef _REENT_SMALL
extern const struct __sFILE_fake __sf_fake_stdin _NLATTRIBUTE ((weak));
extern const struct __sFILE_fake __sf_fake_stdout _NLATTRIBUTE ((weak));
extern const struct __sFILE_fake __sf_fake_stderr _NLATTRIBUTE ((weak));
#endif
