...
struct _reent **__reent_ptrs;
struct _reent *__reent_data;
...
__start(int argc, char* argv[])
{
  ...
  if( RigelIsSim() ) {
    // FIXME Global state should only be touched by thread zero?  We could wrap it with
    ...
    if (tid == 0) ) {
        ...
        uint32_t malloc_start = 0x00400000;
