...
UG *BuildUG(Point3 *v, int n, int m, UG *G)
{
 ...
 box_size = volume/(float)m;
 G->side = powf(box_size*1000,0.333333);
 G->side/=10;
 fprintf(stdout,"box size = %f\n",box_size);
 fprintf(stdout,"in BuildUG G->side = %f\n",G->side);
 SI.CellEdge=G->side;
 ...
 G->C  =  (Plist **)malloc(CellNumber*sizeof(Plist *));
