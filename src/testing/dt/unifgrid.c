...
void ResetUG(UG* G,int n){
 
 int i; 
 G->Mark=0;
 for(i=0;i<n;i++) G->UsedPoint[i]=-1;
 for(i=0;i<G->n;i++) G->Marked[i] = -1;
}
...
UG *BuildUG(Point3 *v, int n, int m, UG *G)
{
 ...
 G->side = powf(volume*1000/(float)m,0.3333333);
 G->side/=10;
 SI.CellEdge=G->side;

 G->x=(int)ceilf((G->vp.x - G->vn.x)/G->side);  /* How many cell per side */
 G->y=(int)ceilf((G->vp.y - G->vn.y)/G->side);
 G->z=(int)ceilf((G->vp.z - G->vn.z)/G->side);
  printf("ceil(%f) = %d\n",(G->vp.x - G->vn.x)/G->side,(int)ceilf((G->vp.x - G->vn.x)/G->side));
 printf("ceil(%f) = %d\n",(G->vp.y - G->vn.y)/G->side,(int)ceilf((G->vp.y - G->vn.y)/G->side));
 printf("ceil(%f) = %d\n",(G->vp.z - G->vn.z)/G->side,(int)ceilf((G->vp.z - G->vn.z)/G->side));
 ...
 G->C  =  (Plist **)malloc(CellNumber*sizeof(Plist *)); // replace with callocs
 G->Marked = (int *)malloc(CellNumber*sizeof(int));
 if(!G->C || !G->Marked) Error("BuildUG, Not enough memory to build UG!!\n",EXIT);
 
 for(i=0;i<CellNumber;i++){ // rigel sim does not initialized to 0
	G->C[i] = NULL;
	G->Marked[CellNumber] = -1;
 }
...
//  printf("adding point %d to cell %d\n",i,index);
  G->C[index] = AddPoint((G->C)[index], i);
 }

 G->Mark=0;

 G->UsedPoint = (int *)malloc(n*sizeof(int));
...
***************************************************************************/
/*
float CalcBestBox(Face *f, Point3 *v, Line *Lc, UG *G,
				IntPoint3 *vn, IntPoint3 *vp, float Radius)
{
 IntPoint3 tvn,tvp;
 int MaxSide;
 int i;
 float MaxRadius,TestRadius,MinRadius,CalcRadius,NewRadius;
...
* MakeLastScan								   *
*									   *
* It do the lasFace *f, Line *Lc, Plane *p, UG *Gt scan of the dataset. See CalcLastScan notes		   *
*									   *
***************************************************************************/
