...
List InCoDe(Point3 *v, int n, double rr_boundary)
{
	List Q=NULL_LIST;
	List T=NULL_LIST;
	List OldFace=NULL_LIST;
  	...
  	while(ExtractList(&f,Q)){
	    ...
	    //		else if((v[t->f[i]->v[0]].x <= rr_boundary) || (v[t->f[i]->v[1]].x <= rr_boundary) || (v[t->f[i]->v[2]].x <= rr_boundary)){
				else{	
					InsertList(t->f[i],Q);
					SI.Face++;
...
//  WritePoints("sorted",v,n);
  ...
  //work only on end-start region
//  printf("middle: %f \n",v[n/2].x);
  T=InCoDe(v,n,v[n/2].x);//v[n-1].x);
